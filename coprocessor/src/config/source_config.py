...
class NTConfigSource:
    ...
    def update(self):
        if not self._init_complete:
            ...
            self._logging_location = ntcore.getStringTopic(
                "logging_location"
            ).subscribe(config.LoggingConfig.logging_location)
            self._event_name = ntcore.getStringTopic("event_name").subscribe(
                config.LoggingConfig.event_name
            )
            self._match_type = ntcore.getStringTopic("match_type").subscribe(
                config.LoggingConfig.match_type
            )
            self._match_number = ntcore.getIntergerTopic("match_number").subscribe(
                config.LoggingConfig.match_number
            )
            self._fiducial_layout = ntcore.getDoubleTopic("fiducial_layout").subscribe(
                config.AprilTagConfig.fiducial_layout
            )
